...
class Thumbnail {
            ...
            switch($this->format) {
                ...
                case 'JPG':
                   # $this->oldImage = ImageCreateFromJpeg($this->fileName);
					try{
						$this->oldImage=$this->callResizeFunction('ImageCreateFromJpeg',$this->fileName);
					} catch (Exception $e) { 
	...
	function callResizeFunction($func,$filename){
		if(!$func($filename)) {
			throw new Exception("We're sorry, we've encountered an error. ");
		} else {			
			$image=$func($filename);
			return $image;
		}
...
/*
 * added functions to interact with this class:
 * author: J.J. Johnson jjohnson@buffnews.com
*/
function adjustWidth($image,$new_width=0){
	/*resize image based on width ONLY */
	...
	$new_height=ceil($ratio * $height);	
	$thumb->resize($new_width,$new_height);
	$thumb->save($image);
}

function adjustHeight($image,$new_height=0){
	/*resize image based on height ONLY */
	$thumb = new Thumbnail($image);
	$width=$thumb->getWidth();
	$height=$thumb->getHeight();
	
	$new_height=($new_height!=0)  ? $new_height  :$height;
	$ratio=$new_height/$height;
	$new_width=ceil($ratio * $width);	
	$thumb->resize($new_width,$new_height);
	$thumb->save($image);
}
?>