...
/**
* function encryptAll
* encrypts an array with the same key
*
*
*
* @param string $key - the key to unlock the encrypted string
* @param mixed $sources - either a string or an array
* @param int $sourcelen - length of the encryption
*
* @return void
*/ 
	function encryptAll($key = "", $sources = "", $sourcelen = 0){
		if(is_array($sources)){
			$outputs = array();
				foreach($sources as $index => $source){
					$outputs[$index] = $this->encrypt($key, $source, $sourcelen);
				}
			return $outputs;
		} else {
			return $this->encrypt($key, $sources, $sourcelen);
		} 
	}
...
/**
* function decryptAll
* decrypts an array with the same key
*
*
*
* @param string $key - the key to unlock the encrypted string
...
* @return void
*/ 
	function decryptAll($key = "", $sources = ""){
		if(is_array($sources)){
			$outputs = array();
				foreach($sources as $index => $source){
					$outputs[$index] = $this->decrypt($key, $source);
				}
			return $outputs;
		} else {
			return $this->decrypt($key, $sources);
		} 
	}
...
* @return void
*/ 
	function encodeAll($sources = ""){
		if(is_array($sources)){
			$outputs = array();
				foreach($sources as $index => $source){
					$outputs[$index] = $this->encode($source);
				}
			return $outputs;
		} else {
			return $this->encrypt($sources);
		} 
	}
...
* @return void
*/ 
	function decodeAll($sources = ""){
		if(is_array($sources)){
			$outputs = array();
				foreach($sources as $index => $source){
					$outputs[$index] = $this->decode($source);
				}
			return $outputs;
		} else {
			return $this->decode($sources);
		} 
	}
